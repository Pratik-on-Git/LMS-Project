...
import { IconBook, IconCategory, IconCategory2, IconChartBar, IconChevronDown, IconClock, IconPlayerPlay } from "@tabler/icons-react";
...
export default async function SlugPage({ params }: { params: { slug: string } }) {
    ...
    // Check enrollment status - forward cookies for authentication
    const enrollmentResponse = await enrollmentApi.checkEnrollmentServer(course.id, cookieHeader);
    const isEnrolled = enrollmentResponse.status === "success" && enrollmentResponse.data?.isEnrolled;

    return (
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3 mt-5">
            <div className="order-1 lg:col-span-2">
                <div className="relative aspect-video w-full overflow-hidden rounded-xl shadow-lg">
                    <Image src={`https://${env.NEXT_PUBLIC_S3_BUCKET_NAME_IMAGES}.t3.storage.dev/${course.fileKey}`} alt={course.title} fill className="object-cover" priority />
                                        ...
                                        <div className="border-t bg-muted/20">
                                            <div className="p-6 pt-4 space-y-3">
                                                {chapter.lessons.map((lesson, lessonIndex) => (
                                                    <div key={lesson.id} className="flex items-center gap-4 rounded-b-lg p-3 hover:bg-muted/50 transition-colors">
                                                        <div className="flex size-8 items-center justify-center rounded-full bg-background border-2 border-primary/15 text-primary font-semibold">
                    ...
                    <Card className="py-0">
                        <CardContent className="p-6">
                                ...
                                <div className="flex flex-col gap-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground">{getLevelDisplay(course.level)}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex flex-col gap-3">
                            ...
                            <div className="mb-6 space-y-3">
                                <h4 className="text-lg font-normal">This Course Includes:</h4>
